...
class Program
{
    ...
    static void FuncionarioMenu()
    {
        ...
        Console.WriteLine("Deseja alterar sua matrícula? (s/n)");
        if (char.ToLower(Console.ReadKey().KeyChar) == 's')
        {
            Console.WriteLine("Digite a nova matrícula:");
            string novaMatricula = Console.ReadLine();
            Console.WriteLine("Digite a senha de administração:");
            string senhaAdmin = Console.ReadLine();
            funcionarioLogado.AlterarMatricula(novaMatricula, senhaAdmin);
        }
...
class Funcionario
{
    ...
    public void AlterarMatricula(string novaMatricula, string senhaAdmin)
    {
        if (senhaAdmin == "senhaAdmin")
        {
            Matricula = novaMatricula;
            Console.WriteLine($"Matrícula alterada para: {novaMatricula}");
        }
        else
        {
            Console.WriteLine("Senha incorreta. Não é possível alterar a matrícula.");
        }
    }
}
